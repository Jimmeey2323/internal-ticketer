...
import { EnhancedTicketCard } from "@/components/enhanced-ticket-card";
...
export default function Dashboard() {
  ...
  // Fetch recent tickets with assignee info
      ...
      const { data, error } = await supabase
        ...
        .select(`
          *,
          ...
          assignedToUser:users!tickets_assignedToUserId_fkey(id, displayName, email),
          assignedDepartment:departments!tickets_assignedDepartmentId_fkey(id, name, code)
        `)
  ...
  // Fetch urgent tickets with assignee info
            ...
            <CardContent className="space-y-3">
              {ticketsLoading ? (
                ...
                recentTickets.map((ticket: any, index: number) => (
                  ...
                  >
                    <EnhancedTicketCard ticket={ticket} showAssignee />
                  </motion.div>
                ))
              ) : (
                <EmptyState
                ...
                {urgentLoading ? (
                  ...
                  urgentTickets.slice(0, 3).map((ticket: any, index: number) => (
                    <motion.div 
                      ...
                      transition={{ delay: index * 0.05 }}
                    >
                      <EnhancedTicketCard ticket={ticket} compact showAssignee />
                    </motion.div>
                  ))
                ) : (
